...
--signal we : std_logic ;
--signal en : std_logic;
--signal addr : std_logic_vector(ram_r-1 downto 0) := (others => '0');
...
--ram: entity work.ram (Behavioral) generic map(w => ram_w , r => ram_r) port map(clk => clk, we => we, en => en, addr => addr, di => di, do => tx_data_in );
